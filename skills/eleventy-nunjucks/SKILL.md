...
| Topic | Framework behavior or project choice |
|---|---|
...
| Output dir | Eleventy default `_site/`; preserve configured `dir.output`. Optional production profile: `out/`. |
| Input | Eleventy default project root; preserve configured `dir.input`. Optional production profile: `src/pages/` or `src/`. |
| Layouts | Resolve from configured layout/include directories; chain via `layout:`. Optional production profile: `src/_includes/layouts/`. |
| Sections / macros | Follow existing include paths. Optional production profile: `src/_includes/sections/…` and `src/_includes/macros/…`. |
| Data | Use configured global-data directory plus directory/template data. Optional production profile: `src/_data/`. See `references/data-cascade.md`. |
...
| [references/production-patterns.md](references/production-patterns.md) | CSP, optional soft-navigation contract, nginx, after-build hooks, OG/JSON-LD patterns |
