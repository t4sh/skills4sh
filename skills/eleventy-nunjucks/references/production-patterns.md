...
Optional deployment recipes for projects that explicitly select them: CSP, View Transitions, theme system, after-build hooks, OG image generation, JSON-LD inlining, sitemap/robots, and the nginx serving model.

These recipes are not Eleventy defaults. Use only the sections matching the existing hosting, routing, CSS, and theme contracts. Custom router events, numbered variants, and theme expiry require explicit project ownership.
...
```nginx
# snippets/security-headers.conf — include wherever add_header is used
...
JSON-LD blocks (`<script type="application/ld+json">…</script>`) are **exempt from `script-src`** because the type is not executable JS. Speculation rules (`<script type="speculationrules">`) are different: allow them with `script-src 'inline-speculation-rules'`, a matching hash, or a nonce. See the [script-src reference](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy/script-src#allowing_speculation_rules).

## Optional custom soft-navigation contract

The `site:*` events below are project-defined router events, not browser View Transitions or Eleventy APIs. Use them only if the router actually dispatches them on the agreed target. Native cross-document view transitions do not require this DOM-swap router.

### Lifecycle events

```js
// theme-boot.js (head, blocking, before paint)
document.addEventListener("site:page-loaded",       reinit);
document.addEventListener("site:before-page-unload", cleanup);
...
```js
import DOMPurify from "/assets/js/vendor/purify.es.mjs";
...
Use the actual vendored ESM build for the import. Preserve style tags only for trusted authored pages and an explicit CSS policy; do not widen the sanitizer for arbitrary remote HTML.

## Optional theme-expiry profile

This product-specific example defaults to light and expires a dark-mode selection six hours after it was selected. It is not an Eleventy recommendation. Preserve the project’s existing system-preference and persistence behavior unless it explicitly chooses expiry. The inline boot script below needs a CSP hash/nonce; alternatively place it in a blocking same-origin external script.
...
Example: src/pages/sitemap.xml.njk
...
{% for item in collections.all %}
  {% if not item.data.sitemap_exclude %}
  <url>
    <loc>{{ site.baseUrl }}{{ item.url | normalize_path }}</loc>
...
Name the loop variable something other than `page`. Reusing `page` shadows Eleventy's global `page` object for the whole loop body. Pages already excluded from collections are absent from `collections.all`, so only a project-specific opt-out key such as `sitemap_exclude` needs checking.
...
server {
  ...
  # Unfingerprinted assets — revalidate; immutable caching needs content-hashed URLs
  location /assets/ {
...
The shared include is the header snippet above. A location containing its own `add_header` does not inherit server-level headers under nginx’s default rules, even with `always`; include the security headers in each such location. See [nginx header inheritance](https://nginx.org/en/docs/http/ngx_http_headers_module.html). Verify a page, asset, redirect, and error response against the actual vhost before deployment.
