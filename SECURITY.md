...
| Branch-protection drift detection | `.github/workflows/branch-protection-drift.yml` runs daily (06:15 UTC) and on every PR; asserts that the live protection config on `refs/heads/main` matches the checked-in expected state at `.github/branch-protection.expected.json`. Activated by setting the `BRANCH_PROTECTION_TOKEN` repo secret to a fine-grained PAT scoped to this repo with `Administration: Read-only` (default `GITHUB_TOKEN` doesn't support that scope). When the secret is absent the workflow exits 0 with setup instructions, so the snapshot remains source of truth either way. |
